  ...
  const handleGenerateVideo = async () => {
    ...
    try {
      ...
      if (error.message === "QUOTA_EXCEEDED") {
        ...
        alert(`视频生成失败：请确保您已选择具备 Veo 权限的付费项目 API Key。`);
      }
    } finally {
      ...
      {quotaError && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6 animate-in fade-in duration-300">
          ...
          <div className="relative glass-card w-full max-w-sm rounded-[32px] p-8 border-purple-500/30 text-center animate-in zoom-in-95">
            ...
            <h3 className="text-xl font-bold mb-4 text-white">API 配额受限 (429)</h3>
            <div className="space-y-4 text-gray-400 text-sm leading-relaxed mb-8">
              <p>检测到您的 Gemini API Key 图像生成配额为 <span className="text-white font-bold">0</span>。</p>
              <div className="bg-white/5 p-4 rounded-2xl flex items-start space-x-3 text-left border border-white/5">
                ...
                <p className="text-[11px]">
                  Google 限制：图像生成模型 (Imagen) 通常需要你在 <b>Google AI Studio</b> 中开启计费 (Pay-as-you-go) 才能激活配额。
                </p>
              </div>
            </div>
      ...
      {currentView === AppView.RESULT && lastResult ? (
        <div className="animate-in fade-in slide-in-from-bottom-4 duration-500">
          ...
          <div className="relative group mb-6">
            {lastResult.videoUrl ? (
              <div className="relative w-full aspect-square rounded-3xl overflow-hidden shadow-2xl bg-black">
                ...
                <div className="absolute top-4 right-4 z-20 flex items-center space-x-1.5 bg-black/60 backdrop-blur-md px-3 py-1.5 rounded-full border border-white/20">
                  ...
                  <span className="text-[10px] font-black tracking-tighter text-white">CINEMATIC VEO</span>
                </div>
              </div>
            ) : (
            ...
            <div className="absolute top-4 left-4 flex flex-col items-start gap-1 z-20">
              <div className="bg-white/10 backdrop-blur-xl border border-white/20 px-3 py-1 rounded-full flex items-center space-x-1.5 shadow-lg">
                ...
                <span className="text-[10px] font-bold tracking-widest text-white uppercase">Selindell Verified</span>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            {!lastResult.videoUrl && (
              <button 
                ...
                className={`w-full py-4 rounded-2xl flex items-center justify-center space-x-3 border-2 transition-all ${
                  isVideoLoading 
                    ? 'border-purple-500/50 bg-purple-500/5 text-purple-400 cursor-wait' 
                    : 'border-purple-500/30 bg-purple-500/10 text-purple-400 hover:bg-purple-500/20 active:scale-95'
                }`}
              >
                {isVideoLoading ? (
                  <>
                    ...
                    <div className="flex flex-col items-start leading-tight">
                      <span className="text-sm font-bold">正在注入灵魂...</span>
                      <span className="text-[10px] opacity-70">Veo 引擎需要 1-3 分钟渲染</span>
                    </div>
                  </>
                ) : (
                  <>
                    <Play size={18} fill="currentColor" />
                    <span className="font-bold">生成电影级展示视频 (VEO)</span>
                  </>
                )}
              </button>
            )}
          ...
          <div className="mt-8 grid grid-cols-2 gap-4">
            <button onClick={() => setView(AppView.HOME)} className="h-14 rounded-2xl border border-white/10 flex items-center justify-center font-bold text-gray-300">
              重新设计
            </button>
            <button 
              onClick={handleOrderNow}
              className="h-14 rounded-2xl purple-gradient shadow-lg shadow-purple-500/30 flex items-center justify-center font-bold active:scale-95 transition-all"
            >
              下单实物手办
            </button>
          </div>
        ...
        <>
          ...
          {/* 重点优化：placeholder 颜色提升，避免看不清 */}
          <div className="glass-card rounded-[32px] p-6 mb-8 relative border-white/10 transition-all focus-within:border-purple-500/40 bg-white/[0.02]">
            <textarea
          ...
          <div className="mb-8">
            <h3 className="text-sm font-bold text-gray-300 uppercase tracking-widest mb-4 px-1">选择风格</h3>
